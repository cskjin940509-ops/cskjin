...
def quote_map(codes):
    ...
    syms = [symbol(c) for c in codes]
    raw = gw.tencent_quotes(syms) if syms else {}
    # Do not let a partial Tushare response mask a fresh Tencent quote for the
    # same stock.  The execution assistant only needs one compliant source.
    out = {}
    for code in codes:
        candidates = [x for x in (primary.get(code), raw.get(symbol(code))) if x]
        if not candidates:
            continue
        candidates.sort(key=lambda x: "".join(ch for ch in str(x.get("quoteTimestamp") or x.get("quoteTime") or "") if ch.isdigit()), reverse=True)
        out[code] = candidates[0]
    return out
