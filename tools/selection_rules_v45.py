...
RULES_ZH = {
    'newEntry': '选股策略保持不变。T日收盘冻结当天候选及排名，T+1按T日收盘价98%低吸限价执行；未触及则14:45–14:55收盘窗口兜底。涨停不可成交最多顺延3个有效交易日。',
    'position': '前一交易日四项掘金指数的综合市场资金情绪决定次日基础仓位，盘中风险只能下调该上限。8批执行层每批预算为信号日收盘总资产的1/8，单股≤2.25%，板块≤30%；选股评分只决定批内排序和资金分配。',
    ...
    'rebalance': '同时保留8个有效信号批次，每批预算为信号日收盘总资产的1/8；第9个批次形成时，T+1开盘轮出最老批次。单股上限2.25%，板块上限30%，单笔至少1万元并遵守100股整手。',
    'exit': 'T日收盘涨停则T+1开盘优先卖出；否则随所属批次在第9个有效信号批次形成后轮出。ATR硬止损、组合风险、T+1、跌停顺延和部分成交待办继续优先执行。',
