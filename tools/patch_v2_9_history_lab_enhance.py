...
private fun SampleGrowthChart29(a: JSONArray?) {
    HCard29 {
        ...
        val values = (0 until a.length()).mapNotNull { i -> a.optJSONObject(i)?.optInt("cumulativeSamples") }
        ...
        Canvas(Modifier.fillMaxWidth().height(130.dp)) {
            if (values.size == 1) {
                ...
                drawPath(path, HBlue29, style = androidx.compose.ui.graphics.drawscope.Stroke(width = 3f))
            }
        }
        ...
        if (n == 0 || a == null || a.length() == 0) {
            ...
            for (i in 0 until a.length()) {
                ...
                Row {
                    ...
                    Text("${x.optInt("sampleCount")}例 · ${p29(d29(x,"share"))}", Modifier.width(90.dp), fontSize = 9.sp)
    ...
    HCard29 {
        ...
        for (i in 0 until a.length()) {
            ...
            Row(verticalAlignment = Alignment.CenterVertically) {
                ...
                LinearProgressIndicator(progress = { share.toFloat().coerceIn(0f,1f) }, Modifier.weight(1f).height(6.dp))
                Spacer(Modifier.width(6.dp))
                Text("${x.optInt("count")} · ${p29(share)}", Modifier.width(66.dp), fontSize = 8.sp)
            }
        }
    }
}
...
private fun HistoryExamplesCard29(o: JSONObject?) {
    HCard29 {
        ...
        fun rows(a: JSONArray?, title: String, color: Color) {
            if (a == null || a.length() == 0) return
            // drawing is handled below because Composables cannot be emitted from a non-composable local helper
        }
        Text("典型成功案例", color = HRed29, fontWeight = FontWeight.Bold, fontSize = 10.sp)
        if (top != null) for (i in 0 until minOf(3, top.length())) {
            val x = top.optJSONObject(i) ?: continue
            Row { Text("${x.optString("name")} ${x.optString("code")}", Modifier.weight(1f), fontSize = 9.sp); Text(p29(d29(x,"fiveDayReturn")), color = c29(d29(x,"fiveDayReturn")), fontSize = 9.sp) }
        }
        HorizontalDivider()
        Text("典型失败案例", color = HGreen29, fontWeight = FontWeight.Bold, fontSize = 10.sp)
        if (bottom != null) for (i in 0 until minOf(3, bottom.length())) {
...
# Insert helpers before the closing Python triple quote that precedes V6 patch logic.
marker = "\n''', encoding='utf-8')\n\nv6 = root / 'V6Activity.kt'"
if marker not in s:
    raise SystemExit('v2.9 generated UI close marker not found')
s = s.replace(marker, append + marker, 1)

p.write_text(s, encoding='utf-8')
print('v2.9 history lab short/long, distributions, clusters and comparisons integrated')
