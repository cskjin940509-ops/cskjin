...
研究版 Android 签名密钥位于 signing/debug.keystore（禁止提交），通过固定 Actions cache 复用；发布时对比上一个双平台版本的签名。缓存丢失或密钥变化时拒绝发布新版本，避免生成无法覆盖安装的更新。生产化发行应将签名密钥迁入维护者管理的持久秘密存储。v4.6及以前流水线未持久保留签名，首次迁移可能需导出本机账本后卸载旧包。
